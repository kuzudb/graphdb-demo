...
def get_betweenness_centrality_records() -> list[dict[str, float]]:
    ...
    bc_records = [
        {"id": node.replace("Account_", ""), "bc": bc[node]} for node in bc
    ]
    return bc_records
...
def update_account_node_table(conn: kuzu.Connection) -> None:
    try:
        conn.execute(
            """
            ALTER TABLE Account ADD bc REAL DEFAULT 0.0
            """
        )
    except RuntimeError:
...
def update_accounts_betweenness_centrality(bc_records: list[dict[str, float]]) -> None:
    ...
    conn.execute(
        ...
        WITH CAST(id AS INT32) AS id, bc
        MERGE (a:Account {id: id})
        ON MATCH SET a.bc = bc
        """
    )
...
async def update_accounts_table(pool: Pool) -> None:
    async with pool.acquire() as conn:
        await conn.execute(
            """
            ALTER TABLE account
            ADD COLUMN IF NOT EXISTS bc REAL DEFAULT 0.0
            """
        )
...
async def insert_betweenness_centrality_records(pool: Pool, records: list[dict]) -> None:
    async with pool.acquire() as conn:
        await conn.executemany(
            "UPDATE account SET bc = $2 WHERE id = $1",
            [(int(record["id"]), record["bc"]) for record in records],
        )
